...
% ภาคผนวก ค.
%*************************************
\appendixChapterTitle{ค}{การติดตั้งและใช้งาน Cloud-Based Platform}
...
\begin{mycustomenum2}
    \item เปิด Terminal หรือ Command Prompt และทำการโคลนโปรเจ็กต์จาก GitHub Repository
...
\end{figure}
    ...
    \item ติดตั้ง Dependencies ของโปรเจ็กต์
...
\end{figure}
    ...
    \item ตั้งค่า Environment Variables โดยคัดลอกไฟล์ตัวอย่าง
...
\adjustbox{frame, width=0.9\textwidth}{
    \begin{minipage}{0.85\textwidth}
        \texttt{\footnotesize
        cp .env.example .env\\
        \# แก้ไขไฟล์ .env ตามการตั้งค่าที่ต้องการ
        }
    \end{minipage}
}
...
\begin{mycustomenum2}
    \item เริ่มต้น Infrastructure Services ด้วย Docker Compose
...
\end{figure}
    ...
    \item เริ่มต้น Midori (Frontend Application)
...
\end{figure}
    ...
    \item เริ่มต้น Momoi (API Service) ใน Terminal ใหม่
...
\end{figure}
    ...
    \item เริ่มต้น Yuzu (VM Operations Service) ใน Terminal ใหม่
...
\section{การใช้งานส่วนประกอบต่างๆ ของระบบ}
\hspace*{1.5em}
...
\begin{mycustomenum2}
    ...
    \begin{mycustomenum2}
        \item ตรวจสอบ Port ที่ใช้งานด้วยคำสั่ง: \texttt{lsof -i :3000} (สำหรับ macOS/Linux)
        \item หยุดกระบวนการที่ใช้ Port หรือเปลี่ยน Port ในไฟล์ \texttt{.env}
    \end{mycustomenum2}

    \item \textbf{Docker Services ไม่สามารถเริ่มได้}
    \begin{mycustomenum2}
        \item ตรวจสอบสถานะด้วยคำสั่ง: \texttt{docker-compose ps}
        \item ดู Log ด้วยคำสั่ง: \texttt{docker-compose logs [service\_name]}
        \item รีสตาร์ท Services ด้วยคำสั่ง: \texttt{docker-compose restart}
    \end{mycustomenum2}
...
\begin{mycustomenum2}
    \item อ่าน Documentation เพิ่มเติมใน \texttt{README.md} ของแต่ละ Component
