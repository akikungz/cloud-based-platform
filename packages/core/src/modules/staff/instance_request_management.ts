...
export class InstanceRequestManagement extends DrizzleAdapter {
	public async getInstanceRequestsByStaffId(
		staff_id: string,
		limit: number = 10,
		offset: number = 0,
	) {
		const result = await this.db
			...
			.innerJoin(this.user, eq(this.instance_request.user, this.user.id))
			.innerJoin(
	...
	public async approveInstanceRequest(requestId: string) {
		...
		if (result.length === 0) {
			throw new Error(
				`Failed to approve instance request with id ${requestId}`,
			);
		}
